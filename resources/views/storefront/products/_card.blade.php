...
<article class="group flex h-full flex-col overflow-hidden rounded-[1.5rem] border border-slate-200/80 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:border-indigo-200 hover:shadow-xl hover:shadow-indigo-100/60">
    <a href="{{ route('products.show', ['slug' => $catalogService->productSlug($product, $currentLanguage), 'language' => $currentLanguage->code, 'currency' => $currentCurrency->code]) }}" class="relative block aspect-[4/3] overflow-hidden bg-gradient-to-br from-slate-100 to-slate-50" aria-label="{{ $name }}">
        @if($image)
    ...
    <div class="flex flex-1 flex-col p-5">
        ...
        <h2 class="min-h-[3.5rem] text-lg font-extrabold leading-7 text-slate-950"><a href="{{ route('products.show', ['slug' => $catalogService->productSlug($product, $currentLanguage), 'language' => $currentLanguage->code, 'currency' => $currentCurrency->code]) }}" class="transition hover:text-indigo-600">{{ $name }}</a></h2>
        ...
        <div class="mt-auto pt-5">
            ...
            @endif
            <a href="{{ route('products.show', ['slug' => $catalogService->productSlug($product, $currentLanguage), 'language' => $currentLanguage->code, 'currency' => $currentCurrency->code]) }}" class="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 py-3 text-sm font-bold text-white transition hover:bg-indigo-600"><span>{{ __('storefront.view_product') }}</span><svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m9 18 6-6-6-6"/></svg></a>
        </div>
    </div>
</article>
